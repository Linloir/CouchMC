...
enum Anchor: String, Codable, CaseIterable, Sendable {
    ...
    /// Center-anchored buttons ignore the `edge` margin (always centered).
...
struct WidgetSpec: Codable, Hashable, Identifiable, Sendable {
    let id: String
    var anchor: Anchor
    var edge: CGFloat       // distance from start/end edge
    var vertical: CGFloat   // distance from top/bottom (or below-center)
...
enum HotbarSwipeMode: String, Codable, CaseIterable, Sendable {
    ...
    case relative       // scroll-wheel: every 32pt of travel cycles ±1
}
...
enum DefaultLayouts {
    ...
    /// Widgets a user can resize / reposition. Joystick is fixed in v1.
